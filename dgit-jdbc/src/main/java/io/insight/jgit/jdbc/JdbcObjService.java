...
public class JdbcObjService implements KVObjectService {
    ...
    private void insertObject(final String repositoryName, final AnyObjectId objectId, final int objectType,
        ...
        final String sql = dsl.insertInto(GIT_OBJECTS, GIT_OBJECTS.OBJECT_ID, GIT_OBJECTS.TYPE,
                GIT_OBJECTS.SIZE, GIT_OBJECTS.REPO, GIT_OBJECTS.BASE, GIT_OBJECTS.DATA,
                GIT_OBJECTS.TOTAL_SIZE).values(objectId.name(), objectType, inflatedSize, repositoryName,
                        base, null, totalSize).onDuplicateKeyIgnore().getSQL();
        final ConnectionProvider provider = dsl.configuration().connectionProvider();
        final Connection conn = provider.acquire();
        try {
